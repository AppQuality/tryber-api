...
describe("POST /users/me/payments", () => {
  ...
  beforeAll(async () => {
    ...
    await tryber.tables.WpAppqUnlayerMailTemplate.do().insert([
      ...
      },
      {
        id: 3,
        html_body: `
          PAYMENT_INVOICE_RECAP_EMAIL_VAT_BODY 
          {Payment.address} 
          {Payment.fiscalType} 
          {Profile.identificationNumber}
          {Profile.bankAccountName}
  ...
  describe("POST /users/me/payments/ - fiscal profile 2", () => {
    ...
    it("Should send an email with {Payment.tax8INPS}", async () => {
      const response = await request(app)
        .post("/users/me/payments")
        .send({
          method: {
  ...
  describe("POST /users/me/payments/ - fiscal profile 3", () => {
    ...
    it("Should send an email with {Payment.tax4INPS}", async () => {
      const response = await request(app)
        .post("/users/me/payments")
        .send({
          method: {
  ...
  describe("POST /users/me/payments/ - fiscal profile 5", () => {
    ...
    it("Should send an email with {Payment.taxWitholdingCompany}", async () => {
      const response = await request(app)
        .post("/users/me/payments")
        .send({
          method: {
    ...
    it("Should send an email with {Payment.taxCompany}", async () => {
      const response = await request(app)
        .post("/users/me/payments")
        .send({
          method: {
